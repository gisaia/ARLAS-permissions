...
if [ -z "${CP}" ]; then
  CP="./arlas-permissions-server.jar"
fi

java -Xmx${ARLAS_XMX} -XX:+ExitOnOutOfMemoryError ${JVM_OPTION} -cp "${CP}" io.arlas.permissions.server.app.ArlasPermissionsServer server /opt/app/configuration.yaml
